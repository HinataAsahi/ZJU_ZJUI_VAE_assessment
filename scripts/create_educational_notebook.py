...
cells = [
    ...
    markdown(
        ...
        "训练通常最小化 `reconstruction + beta * KL`，它等价于最大化 ELBO 的负号版本。两个目标会拉扯：重构项希望每个 `z` 保留足够多的输入细节；KL 项希望每个 `q(z|x)` 不要偏离统一的先验 `N(0,I)` 太远。\n"
    ),
    markdown(
    ...
    markdown(
        ...
        "`beta=1` 是标准 VAE 目标；`beta=0` 完全移除 KL 对 total loss 的影响。公平比较时只改变 beta，保持数据、网络、seed、epoch、batch size 和学习率一致。\n"
    ),
    markdown(
